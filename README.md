...
# What is Boyer-Moore Horspool algorithm
  ...
  # Then what ?
  Then your program is not searching the find word char by char but a jumping score. It works like; let's say your trying to find the word "whocares". This jump method repeats until it finds the last character of our word. Then rewind it. So we need to find 's' in our text. Let's say our text be ajahoekrewhocaresbbbb , now we need to find s in it. To find our last char, that means we can start to look for from |whocares| = 8 index. But first we need to create our table to get jump scores for each character. 
  ...
  # Authentication for the above example
    Inserting a 's' to point where I am showing.
    ajahoekrewhocaresbbbb -> Starting from the index 8.
           s              -> found 'r' jump 2.
    ajahoekrewhocaresbbbb
             s            -> found 'w' jump 7.
    ajahoekrewhocaresbbbb
                    s     -> found 's' try to match words.
    ajahoekrewhocaresbbbb
